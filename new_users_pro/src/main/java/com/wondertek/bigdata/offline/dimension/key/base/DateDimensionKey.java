 ...
 * 日期维度类
 */
public class DateDimensionKey extends KeyBaseDimension {
    /**
     * 数据库主键
     */
    private int id;
     ...
     *
     * @param time 毫秒级时间戳
     * @param type 需要创建的日期维度类型
     * @return 返回一个对应的时间维度对象
     * @throws RuntimeException 如果给定的type没法创建，那么直接抛出此异常
     */
    public static DateDimensionKey buildDate(long time, DateEnum type) {
        // 创建一个日历对象
        ...
        if (DateEnum.YEAR.equals(type)) {
            ...
            return new DateDimensionKey(year, 0, 0, 0, 0, type.name, calendar.getTime());
        }
        ...
        if (DateEnum.SEASON.equals(type)) {
            ...
            return new DateDimensionKey(year, season, 0, 0, 0, type.name, calendar.getTime());
        }
        ...
        if (DateEnum.MONTH.equals(type)) {
            ...
            return new DateDimensionKey(year, season, month, 0, 0, type.name, calendar.getTime());
        }
        ...
        if (DateEnum.WEEK.equals(type)) {
            ...
            return new DateDimensionKey(year, season, month, week, 0, type.name,
        ...
        if (DateEnum.DAY.equals(type)) {
            ...
            return new DateDimensionKey(year, season, month, week, day, type.name, calendar.getTime());
        }
     ...
     * @param day      天
     * @param type     日期维度类型
     * @param calendar 具体对于的日期
     */
    public DateDimensionKey(int id, int year, int season, int month, int week, int day, String type,
                            Date calendar) {
     ...
     * @param calendar 具体对于的日期
     */
    public DateDimensionKey(int year, int season, int month, int week, int day, String type,
                            Date calendar) {
        this.year = year;
        this.season = season;
        this.month = month;
    ...
    public boolean equals(Object obj) {
        ...
        DateDimensionKey other = (DateDimensionKey) obj;
        if (calendar == null) {
    ...
    public int compareTo(KeyBaseDimension o) {
        ...
        DateDimensionKey dd = (DateDimensionKey) o;
