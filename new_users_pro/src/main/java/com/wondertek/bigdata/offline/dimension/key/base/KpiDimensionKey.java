package com.wondertek.bigdata.offline.dimension.key.base;

import com.wondertek.bigdata.offline.dimension.key.KeyBaseDimension;

import java.io.DataInput;
import java.io.DataOutput;
import java.io.IOException;

/**
 * kpi维度类
 */
public class KpiDimensionKey extends KeyBaseDimension {

    /**
     * 数据库主键id
     */
   // private int id;
    /**
     * kpi名称
     */
    private String kpiName;

    /**
     * 无参构造函数，必须给定
     */
    public KpiDimensionKey() {
        super();
    }

    /**
     * 给定具体参数的构造函数
     *
     * @param id
     * @param kpiName
     */
    public KpiDimensionKey(int id, String kpiName) {
        super();
      //  this.id = id;
        this.kpiName = kpiName;
    }

    /**
     * 给定kpi名称的构造函数
     *
     * @param kpiName kpi名称
     */
    public KpiDimensionKey(String kpiName) {
        super();
        this.kpiName = kpiName;
    }

    @Override
    public void write(DataOutput out) throws IOException {
        //out.writeInt(id);
        out.writeUTF(kpiName);
    }

    @Override
    public void readFields(DataInput in) throws IOException {
        //this.id = in.readInt();
        this.kpiName = in.readUTF();
    }

    @Override
    public int compareTo(KeyBaseDimension o) {
        if (o == this) {
            return 0;
        }

        KpiDimensionKey kd = (KpiDimensionKey) o;

       // int result = Integer.compare(this.id, kd.getId());
       // if (result == 0) {
            return this.kpiName.compareTo(kd.getKpiName());
       // }
       // return result;
    }

//    public int getId() {
//        return id;
//    }

//    public void setId(int id) {
//        this.id = id;
//    }

    public String getKpiName() {
        return kpiName;
    }

    public void setKpiName(String kpiName) {
        this.kpiName = kpiName;
    }
}
